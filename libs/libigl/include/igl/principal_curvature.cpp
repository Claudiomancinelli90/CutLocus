...
public:
  ...
  public:
    ...
    IGL_INLINE double duv(double u, double v) { return b(); }

    IGL_INLINE double duu(double u, double v) { return 2.0 * a(); }

    IGL_INLINE double dvv(double u, double v) { return 2.0 * c(); }

    IGL_INLINE static Quadric fit(const std::vector<Eigen::Vector3d> &VV) {
      assert(VV.size() >= 5);
      if (VV.size() < 5) {
        std::cerr
            << "ASSERT FAILED! fit function requires at least 5 points: Only "
            << VV.size() << " were given." << std::endl;
        exit(0);
      }
...
CurvatureCalculator::fitQuadric(const Eigen::Vector3d &v,
  ...
  if (points.size() < 5) {
    std::cerr << "ASSERT FAILED! fit function requires at least 5 points: Only "
              << points.size() << " were given." << std::endl;
    *q = Quadric(0, 0, 0, 0, 0);
  } else {
...
template <typename DerivedV, typename DerivedF, typename DerivedPD1,
          ...
          typename Index>
IGL_INLINE void
igl::principal_curvature(const Eigen::PlainObjectBase<DerivedV> &V,
                         ...
                         std::vector<Index> &bad_vertices, unsigned radius,
                         bool useKring) {
...
igl::principal_curvature(const Eigen::PlainObjectBase<DerivedV> &V,
  ...
  for (unsigned i = 0; i < V.rows(); ++i) {
    ...
    if (PD1.row(i) * PD2.row(i).transpose() > 10e-6) {
      std::cerr << "PRINCIPAL_CURVATURE: Something is wrong with vertex: " << i
                << std::endl;
      PD1.row(i) *= 0;
      PD2.row(i) *= 0;
    }
