...
IGL_INLINE bool
igl::heat_geodesics_precompute(const Eigen::MatrixBase<DerivedV> &V,
...
IGL_INLINE bool igl::heat_geodesics_precompute(
    ...
    const Scalar t, HeatGeodesicsData<Scalar> &data) {
  ...
  if (data.use_intrinsic_delaunay) {
    ...
    igl::massmatrix_intrinsic(l_intrinsic, F_intrinsic, MASSMATRIX_TYPE_DEFAULT,
                              M);
  ...
  data.Div = -0.25 * data.Grad.transpose() *
             dblA.colwise().replicate(data.ng).asDiagonal();
  ...
  igl::unique(O, data.b);
  {
    ...
    if (!igl::min_quad_with_fixed_precompute(Q, Eigen::VectorXi(), _, true,
                                             data.Neumann)) {
      return false;
    }
    // Only need if there's a boundary
    if (data.b.size() > 0) {
      if (!igl::min_quad_with_fixed_precompute(Q, data.b, _, true,
                                               data.Dirichlet)) {
        return false;
      }
    }
    const Eigen::SparseMatrix<double> Aeq =
        M.diagonal().transpose().sparseView();
    L *= -0.5;
    if (!igl::min_quad_with_fixed_precompute(L, Eigen::VectorXi(), Aeq, true,
                                             data.Poisson)) {
      return false;
    }
  }
...
IGL_INLINE void
igl::heat_geodesics_solve(const HeatGeodesicsData<Scalar> &data,
  ...
  // Neumann solution
  DerivedD u;
  igl::min_quad_with_fixed_solve(data.Neumann, u0, DerivedD(), DerivedD(), u);
  if (data.b.size() > 0) {
    // Average Dirichelt and Neumann solutions
    DerivedD uD;
    igl::min_quad_with_fixed_solve(data.Dirichlet, u0,
                                   DerivedD::Zero(data.b.size()).eval(),
                                   DerivedD(), uD);
    u += uD;
    u *= 0.5;
  }
  ...
  igl::min_quad_with_fixed_solve(data.Poisson, (-2.0 * div_X).eval(),
                                 DerivedD(), Beq, D);
  DerivedD Dgamma;
...
template void
igl::heat_geodesics_solve<double, Eigen::Matrix<int, -1, 1, 0, -1, 1>,
                          Eigen::Matrix<double, -1, 1, 0, -1, 1>>(
    igl::HeatGeodesicsData<double> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, 1, 0, -1, 1>> const &,
    Eigen::PlainObjectBase<Eigen::Matrix<double, -1, 1, 0, -1, 1>> &);
template bool
igl::heat_geodesics_precompute<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                               Eigen::Matrix<int, -1, -1, 0, -1, -1>, double>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &, double,
    igl::HeatGeodesicsData<double> &);
template bool
igl::heat_geodesics_precompute<Eigen::Matrix<double, -1, -1, 0, -1, -1>,
                               Eigen::Matrix<int, -1, -1, 0, -1, -1>, double>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, -1, 0, -1, -1>> const &,
