...
IGL_INLINE void grad_tet(const Eigen::MatrixBase<DerivedV> &V,
  ...
  Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, Eigen::Dynamic> N(
      F.rows(), 3);
  if (!uniform) {
    ...
    // Use a uniform tetrahedra as a reference, with the same volume as the
    // original one:
    //
    ...
    for (int i = 0; i < m; i++) {
      ...
      double a =
          ...
          std::cbrt(
              3 *
              vol(i)); // area of a face in a uniform tet with volume = vol(i)
  ...
  for (int i = 0; i < 4 * m; i++) {
    ...
    G_t.push_back(
        Triplet<double>(0 * m + i_idx, j_idx, val_before_n * N(i, 0)));
    G_t.push_back(
        Triplet<double>(1 * m + i_idx, j_idx, val_before_n * N(i, 1)));
    G_t.push_back(
        Triplet<double>(2 * m + i_idx, j_idx, val_before_n * N(i, 2)));
  }
  G.resize(3 * m, n);
...
IGL_INLINE void grad_tri(const Eigen::MatrixBase<DerivedV> &V,
  ...
  Eigen::Matrix<typename DerivedV::Scalar, Eigen::Dynamic, 3> eperp21(m, 3),
      eperp13(m, 3);
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (!uniform) {
      ...
      // Abstract equilateral triangle v1=(0,0), v2=(h,0), v3=(h/2,
      // (sqrt(3)/2)*h)
      ...
      double h = std::sqrt(
          (dblA) /
          std::sin(igl::PI /
                   3.0)); // (h^2*sin(60))/2. = Area => h = sqrt(2*Area/sin_60)
...
template void igl::grad<Eigen::Matrix<double, -1, 2, 0, -1, 2>,
                        Eigen::Matrix<int, -1, -1, 0, -1, -1>>(
    Eigen::MatrixBase<Eigen::Matrix<double, -1, 2, 0, -1, 2>> const &,
    Eigen::MatrixBase<Eigen::Matrix<int, -1, -1, 0, -1, -1>> const &,
    Eigen::SparseMatrix<Eigen::Matrix<double, -1, 2, 0, -1, 2>::Scalar, 0, int>
        &,
    bool);
