...
void init_arraybuffer(Arraybuffer &buffer, const vector<float> &array,
                      bool dynamic);

template <typename T>
void init_arraybuffer(Arraybuffer &buffer, const vector<T> &array,
                      bool dynamic = false) {
  buffer = Arraybuffer{};
  buffer.num = (int)size(array);
...
template <typename Type> struct Uniform {
  const char *name;
  Type value;
...
template <typename T>
inline void set_uniform(const Shader &shader, const char *name,
                        const T &value) {
...
template <typename T>
inline void set_uniform(const Shader &shader, const char *name, const T *values,
                        int num_values) {
...
void set_uniform_texture(int location, int location_on, const Texture &texture,
                         int unit);
void set_uniform_texture(const Shader &shader, const char *name,
...
void set_vertex_attribute(const Shader &shader, const char *name,
                          const Type &buffer) {
...
template <typename Type> void draw_points(const vector<Type> &points) {
...
int add_vertex_attribute(Shape &shape, const vector<T> &data) {
  ...
  bind_shape(shape);
  int index = (int)shape.vertex_attributes.size();
  auto &attribute = shape.vertex_attributes.emplace_back();
  init_arraybuffer<T>(attribute, data);
  set_vertex_attribute(index, attribute);
  return index;
}

template <typename T> int add_vertex_attribute(Shape &shape, const T &data) {
  bind_shape(shape);
  int index = (int)shape.vertex_attributes.size();
...
Shape make_polyline_shape(const vector<vec3f> &position,
...
void draw_shape(const Shape &shape, const Shader &shader,
                const Uniform<Args> &... args) {
  bind_shader(shader);
  set_uniform(shader, args...);
  draw_shape(shape);
...
Camera make_lookat_camera(const vec3f &from, const vec3f &to,
