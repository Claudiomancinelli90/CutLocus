...
void init_texture(Texture &texture, const image_data &img, bool as_float,
                  bool linear, bool mipmap) {
  init_texture(texture, vec2i{img.width, img.height}, as_float, false, linear,
               mipmap);
  // update_texture(texture, img, mipmap);
}

// void init_texture(Texture &texture, const image_data &img, bool as_srgb,
//                   bool linear, bool mipmap) {
//   init_texture(texture, vec2i{img.width, img.height}, false, as_srgb, linear,
//                mipmap);
//   update_texture(texture, img, mipmap);
// }

// void update_texture(Texture &texture, const image_data &img, bool mipmap) {
//   check_error();
//   glBindTexture(GL_TEXTURE_2D, texture.id);
...
void set_uniform_texture(int location, int locatiom_on, const Texture &texture,
                         int unit) {
  check_error();
  if (texture.id) {
    glActiveTexture(GL_TEXTURE0 + unit);
...
void set_uniform_texture(const Shader &shader, const char *name,
                         const char *name_on, const Texture &texture,
                         int unit) {
...
void set_vertex_attribute(int location, const Arraybuffer &buffer) {
  ...
  glVertexAttribPointer(location, buffer.elem_size / 4, GL_FLOAT, false, 0,
                        nullptr);
  check_error();
}

void set_vertexattrib(const Shader &shader, const char *name,
                      const vec3f &value) {
...
void draw_image(const Texture &texture, int win_width, int win_height,
  ...
  set_uniform(gl_prog, "window_size",
...
void draw_image_background(const Texture &texture, int win_width,
  ...
  if (!gl_prog) {
    ...
    init_shader(gl_prog, true);
    init_arraybuffer(gl_texcoord, vector<vec2f>{{0, 0}, {0, 1}, {1, 1}, {1, 0}},
                     false);
  ...
  set_uniform(gl_prog, "image_size",
              vec2f{(float)texture.size.x, (float)texture.size.y});
  set_uniform(gl_prog, "border_size",
...
Shape make_polyline_shape(const vector<vec3f> &positions,
...
Shape make_quad_shape() {
  auto shape = Shape{};
  init_shape(shape);
  add_vertex_attribute(shape,
                       vector<vec2f>{{-1, -1}, {1, -1}, {-1, 1}, {1, 1}});
...
Shape make_vector_field_shape(const vector<vec3f> &vector_field,
                              const vector<vec3f> &from, float scale) {
...
Shape make_vector_field_shape(const vector<vec3f> &vector_field,
                              ...
                              const vector<vec3f> &positions, float scale) {
...
void draw_shape(const Shape &shape) {
  ...
  if (!shape.id)
    return;
  if (shape.vertex_attributes.empty())
    return;
  ...
  if (primitives) {
    ...
    if (shape.is_strip) {
      if (!positions)
        return;
      if (shape.type == Shape::type::points)
        draw_point_array(positions);
      if (shape.type == Shape::type::lines)
        draw_line_strip(positions);
      if (shape.type == Shape::type::triangles)
        draw_triangle_strip(positions);
    } else {
      if (shape.type == Shape::type::points)
        draw_point_array(positions);
      if (shape.type == Shape::type::lines)
        draw_line_array(positions);
      if (shape.type == Shape::type::triangles)
        draw_triangle_array(positions);
    }
    check_error();
  }
}
...
Rendertarget make_render_target(const vec2i &size, bool as_float, bool as_srgb,
                                bool linear, bool mipmap) {
...
image_data capture_screenshot() {
  ...
  auto img = image_data({width, height});
  // char *data = (char*) malloc((size_t) (width * height * 3)); // 3 components
  // (R, G, B)

  glPixelStorei(GL_PACK_ALIGNMENT, 1);
  // glReadPixels(x, y, width, height, GL_RGBA, GL_FLOAT, img.operator[]);
